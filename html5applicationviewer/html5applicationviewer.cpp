    ...
    : QGraphicsView(parent)
{
    ...
    connect(m_webView,
            SIGNAL(loadFinished(bool)), SLOT(loadFinished()));
}
...
void Html5ApplicationViewerPrivate::addToJavaScript()
{
    ...
    m_webView->page()->mainFrame()->addToJavaScriptWindowObject("driverFactory",
                                                                new DriverInstanceFactory,
                                                                QWebFrame::ScriptOwnership);
}

void Html5ApplicationViewerPrivate::loadFinished()
{
    QFile file;
    QSettings settings(adjustPath("settings/settings.ini"), QSettings::IniFormat);
    QString login = settings.value("General/login", "").toString();
    QString password = settings.value("General/password", "").toString();

    m_webView->page()->mainFrame()->evaluateJavaScript(QString(
                "var login=\"%1\";\n"
                "var password=\"%2\";").arg(login).arg(password));

    file.setFileName(adjustPath("js/login.js"));
    file.open(QIODevice::ReadOnly);
    m_webView->page()->mainFrame()->evaluateJavaScript(file.readAll());
    file.close();

    file.setFileName(adjustPath("js/drivers.js"));
    file.open(QIODevice::ReadOnly);
    m_webView->page()->mainFrame()->evaluateJavaScript(file.readAll());
